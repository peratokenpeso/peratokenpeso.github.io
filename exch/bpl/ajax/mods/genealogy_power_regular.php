...
namespace BPL\Ajax\Mods\Power\Genealogy\Regular;
...
function main($id_user, $plan)
{
	...
	details_power_regular(user($id_user), $plan);
...
function make_json($parent, $plan)
{
	...
	if (!empty($children))
	{
		foreach ($children as $child)
		{
			...
			details_power_regular($child, $plan);
 ...
 * @since version
 */
function details_power_regular($child, $plan): void
{
	echo '"id": "' . $child->id . '", ';
...
{
	$power_id = user_power_regular($user_id)->id;

	return fetch_all(
		'SELECT * ' .
		'FROM network_power_regular a ' .
 ...
 * @since version
 */
function user_power_regular($user_id)
{
	return fetch(
		'SELECT * ' .
		'FROM network_power_regular ' .
