...
namespace BPL\Ajax\Mods\Power\Genealogy\Associate;
...
function main($id_user, $plan)
{
	...
	details_power_associate(user($id_user), $plan);
...
function make_json($parent, $plan)
{
	...
	if (!empty($children))
	{
		foreach ($children as $child)
		{
			...
			details_power_associate($child, $plan);
 ...
 * @since version
 */
function details_power_associate($child, $plan): void
{
	echo '"id": "' . $child->id . '", ';
...
{
	$power_id = user_power_associate($user_id)->id;

	return fetch_all(
		'SELECT * ' .
		'FROM network_power_associate a ' .
 ...
 * @since version
 */
function user_power_associate($user_id)
{
	return fetch(
		'SELECT * ' .
		'FROM network_power_associate ' .
