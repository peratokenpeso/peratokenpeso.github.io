...
namespace BPL\Ajax\Mods\Power\Genealogy\Executive;
...
function main($id_user, $plan)
{
	...
	details_power_executive(user($id_user), $plan);
...
function make_json($parent, $plan)
{
	...
	if (!empty($children))
	{
		foreach ($children as $child)
		{
			...
			details_power_executive($child, $plan);
 ...
 * @since version
 */
function details_power_executive($child, $plan): void
{
	echo '"id": "' . $child->id . '", ';
...
{
	$power_id = user_power_executive($user_id)->id;

	return fetch_all(
		'SELECT * ' .
		'FROM network_power_executive a ' .
 ...
 * @since version
 */
function user_power_executive($user_id)
{
	return fetch(
		'SELECT * ' .
		'FROM network_power_executive ' .
